var htmx = (function () {
    ...
    function processWasm(elt, nodeData, triggerSpecs) {
        let explicitAction = false
        // forEach(VERBS, function (verb) {
        if (hasAttribute(elt, 'hx-wasm')) {
            const path = getAttributeValue(elt, 'hx-wasm')
            explicitAction = true
            nodeData.path = path
            //nodeData.verb = verb
            triggerSpecs.forEach(function (triggerSpec) {
                addTriggerHandler(elt, triggerSpec, nodeData, function (node, evt) {
                    ...
                    //alert("arriba teran!");
                    issueWasmRequest('get', path, elt, evt)
                })
            })
        }
        // }
    ...
    function findElementsToProcess(elt) {
        if (elt.querySelectorAll) {
            ...
            const results = elt.querySelectorAll(VERB_SELECTOR + boostedSelector + ", form, [type='submit']," +
                ' [hx-ext], [data-hx-ext], [hx-trigger], [data-hx-trigger], [hx-wasm]' + extensionSelectors.flat().map(s => ', ' + s).join(''))
    ...
    function initNode(elt) {
        ...
        if (nodeData.initHash !== attributeHash(elt)) {
            ...
            const hasWasmCall = processWasm(elt, nodeData, triggerSpecs)
   ...
   */
    ...
    function issueWasmRequest(verb, path, elt, event, etc, confirmed) {
        let resolve = null
        let reject = null
        etc = etc != null ? etc : {}
        if (etc.returnPromise && typeof Promise !== 'undefined') {
        ...
        const responseHandler = etc.handler || handleWasmResponse
        const select = etc.select || null
    ...
    function handleWasmResponse(elt, responseInfo) {
        //const xhr = responseInfo.xhr
        let target = responseInfo.target
        const etc = responseInfo.etc
        const responseInfoSelect = responseInfo.select
